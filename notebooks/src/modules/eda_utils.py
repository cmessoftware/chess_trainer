...
def clean_and_prepare_dataset(df):
    ...
    df.dropna(subset=["game_id", "move_number",
              "fen", "move_uci"], inplace=True)
...
def group_summary(df):
    ...
    print(df.groupby("phase")["error_label"].value_counts(
        normalize=True).unstack().round(2))

    print("\nEstadísticas de movilidad por color:")
    print(df.groupby("player_color")[
          ["self_mobility", "opponent_mobility"]].describe())
    ...
    print(df.pivot_table(values="error_label",
          index="is_center_controlled", aggfunc="count"))
