...
class RealTimeChessPredictor:
    ...
    def __init__(self, model_name="ChessErrorClassifier", version="latest"):
        ...
        if not self.predictor.load_model(model_name=model_name, version=version):
            logger.error("No se pudo cargar el modelo. Asegúrate de haber entrenado uno primero.")
        ...
        try:
            ...
            if get_real_score and self.engine:
                try:
                    ...
                    real_score_diff = abs(score_after - score_before) if score_before and score_after else None
                    ...
                    result.update({
                        "stockfish_evaluation": {
                            ...
                            "prediction_correct": prediction["predicted_error"] == real_error
                        }
                    })
    ...
    def analyze_game_moves(self, pgn_string, max_moves=None):
        ...
        try:
            ...
            if moves_analysis:
                predictions = [m["predicted_error"] for m in moves_analysis if "predicted_error" in m]
                correct_predictions = [
                    m["stockfish_evaluation"]["prediction_correct"] 
                    for m in moves_analysis 
                    if "stockfish_evaluation" in m and "prediction_correct" in m["stockfish_evaluation"]
                ]
                
                stats = {
                    "total_moves_analyzed": len(moves_analysis),
                    "error_distribution": {
                        error: predictions.count(error) for error in ["good", "inaccuracy", "mistake", "blunder"]
                    },
                    "prediction_accuracy": sum(correct_predictions) / len(correct_predictions) if correct_predictions else None
                }
            else:
...
def demo_prediction():
    ...
    if "moves_analysis" in game_analysis:
        logger.info(f"Jugadas analizadas: {game_analysis['game_statistics']['total_moves_analyzed']}")
        logger.info("Distribución de errores predichos:")
        for error, count in game_analysis['game_statistics']['error_distribution'].items():
            logger.info(f"  {error}: {count}")
        
        if game_analysis['game_statistics']['prediction_accuracy']:
            logger.info(f"Precisión de predicciones: {game_analysis['game_statistics']['prediction_accuracy']:.4f}")
