...
cells = [
    {
        ...
        "source": [
            ...
            "\n",
            "Starter model: **Random Forest** + **StratifiedGroupKFold** by `game_id`.\n",
            "\n",
            "- **OOF macro-F1** (3-fold GroupKFold on train): ~0.61 — local CV estimate\n",
            "- **Test macro-F1** (full train → submission): ~0.67–0.71 — leaderboard benchmark\n",
            "\n",
    ...
    },
    {
        ...
        "source": [
            ...
            "    \"\"\"Find competition CSV folder on Kaggle or locally.\"\"\"\n",
            "    candidates: list[Path] = []\n",
            "    kaggle_input = Path('/kaggle/input')\n",
            "    if kaggle_input.exists():\n",
            "        for path in sorted(kaggle_input.rglob('train.csv')):\n",
            "            candidates.append(path.parent)\n",
            "        # Competition data is sometimes mounted at /kaggle/input/*.csv (no subfolder)\n",
            "        if (kaggle_input / 'train.csv').exists():\n",
            "            candidates.insert(0, kaggle_input)\n",
            "    local = Path('output')\n",
            ...
            "    if not candidates:\n",
            "        hint = '\\n'.join(\n",
            "            str(p) for p in sorted(kaggle_input.rglob('*'))[:30]\n",
            "        ) if kaggle_input.exists() else 'not on Kaggle'\n",
            "        raise FileNotFoundError(\n",
            "            'train.csv not found. Add the competition dataset to this notebook '\n",
            "            '(Add Data → Competition or your ChessTrainer dataset).\\n'\n",
            "            f'Checked under: {kaggle_input}\\nListing:\\n{hint}'\n",
            "        )\n",
            "    return candidates[0]\n",
            "\n",
            "\n",
