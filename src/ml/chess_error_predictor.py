...
class ChessErrorPredictor:
    ...
    def __init__(self):
        ...
        self.feature_columns = [
            'score_diff', 'material_balance', 'material_total', 'num_pieces',
            'branching_factor', 'self_mobility', 'opponent_mobility', 'move_number',
            'player_color', 'has_castling_rights', 'is_repetition', 'is_low_mobility',
            'is_center_controlled', 'is_pawn_endgame', 'threatens_mate', 'is_forced_move'
        ]
    ...
    def load_training_data(self):
        ...
        logger.info(f"Distribución de errores: {features_df['error_label'].value_counts().to_dict()}")
    ...
    def train_model(self, experiment_name="chess_error_prediction_v2"):
        ...
        with mlflow.start_run(run_name="random_forest_optimized") as run:
            ...
            for label, count in class_distribution.items():
                mlflow.log_param(f"class_{self.label_encoder.classes_[label]}_count", count)
    ...
    def load_model(self, run_id=None, model_name="ChessErrorClassifier", version="latest"):
    ...
    def batch_predict(self, features_df):
        ...
        results['predicted_error'] = [self.label_encoder.classes_[p] for p in predictions]
