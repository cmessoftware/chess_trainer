...
from db.db_utils import load_analyzed_tacticals_hashes, save_analyzed_tacticals_hash
...
def run_parallel_analysis_from_db(max_workers=4):
    ...
    print(f"🔍 Cargando partidas ya analizadas... {len(analyzed)} encontradas: {analyzed}")

    pending_games = [g for g in all_games if repo.get_game_id_by_game(g) not in analyzed]
    print(f"🚀 Ejecutando análisis paralelo sobre {len(pending_games)} partidas...")
    
    #Para testing
    limit = 1
    pending_games = pending_games[:limit]  # Limitar a 1 partidas para depuración
    print(f"🔍 Análisis limitado a {len(pending_games)} partidas para depuración")

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(analyze_game_parallel, game) for game in pending_games]

        for future in as_completed(futures):
            game_hash, tags_df = future.result()
            print(f"🔍 Procesando partida {game_hash}...de longitud {len(game_hash)}")
...
def analyze_game_parallel(game):
    try:
        ...
        print(f"❌ Error en partida {game.headers.get('White')} vs {game.headers.get('Black')}: {e}\n{traceback.format_exc()}")
        return (None, None)
