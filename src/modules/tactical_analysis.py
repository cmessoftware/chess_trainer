...
from db.db_utils import get_game_by_id, init_analyzed_tacticals_table, load_analyzed_tacticals_hashes, save_analyzed_tacticals_hash
...
def analyze_single_game_tactics(game_id):
    ...
    try:
        ...
        print(f"❌ Excepcion al analizar partida {game_id} - {game.headers.get('White', '?')} vs {game.headers.get('Black', '?')}: {e} - {traceback.print_exc()}")
        if e.__cause__:
...
def detect_tactics_from_game(game, depth=10):  # Detecta patrones tácticos en una partida de ajedrez. Bajo depth=15 a 10 para acelerar el análisis
    try:
        ...
        for i, move in enumerate(game.mainline_moves()):
            ...
            min_branching_for_analysis = TACTICAL_ANALYSIS_SETTINGS.get("min_branching_for_analysis", 4)
            
            if len(list(board.legal_moves)) <= min_branching_for_analysis:
                print(f"⏭️ Jugada #{i+1} omitida por baja complejidad (branching < {min_branching_for_analysis})")
                board.push(move)
                continue
            ...
            if fen_before in eval_cache:
                ...
                print(f"Evaluando FEN antes del movimiento: {fen_before}")
                eval_before = get_evaluation(fen_before, depth,multipv=multipv)
           ...
           # ➤ Analizar con MultiPV si hay alternativas mejores
            if "best" in eval_before:
                tag_alt = compare_to_best(eval_before["best"], eval_before.get("alternatives", []), threshold_cp=100)
            else:
            ...
            print(f"Evaluacion completa para el movimiento {board.turn}:{i+1} : {move.uci()}")
        return tags
    except Exception as e:
