...
class SmartGamesFetcher:
    ...
    def _load_existing_games(self):
        ...
        try:
            if DB_URL:
                ...
                conn.close()
                logger.info(f"📊 Loaded {len(self.known_game_ids)} existing game IDs from database")
        except Exception as e:
            logger.warning(f"⚠️ Could not load existing games from database: {e}")
    ...
    def _extract_game_metadata_from_pgn(self, pgn_text: str, platform: str) -> Optional[GameMetadata]:
        ...
        try:
            ...
            game_type, formatted_time_control = self._parse_time_control(time_control)
    ...
    def _fetch_lichess_games(self, username: str, max_games: int, since_date: Optional[str] = None) -> List[GameMetadata]:
        ...
        logger.debug(f"🔍 Fetching Lichess games for {username} (max: {max_games})")
        ...
        try:
            ...
            for pgn_text in pgn_games:
                ...
                metadata = self._extract_game_metadata_from_pgn(pgn_text, "lichess")
                if metadata:
                    games.append(metadata)
                    self.known_game_ids.add(metadata.game_id)
            
        except Exception as e:
            logger.warning(f"⚠️ Error processing Lichess games for {username}: {e}")
    ...
    def _fetch_chesscom_games(self, username: str, max_games: int, since_date: Optional[str] = None) -> List[GameMetadata]:
        ...
        logger.debug(f"🔍 Fetching Chess.com games for {username} (max: {max_games})")
        ...
        try:
            ...
            for archive_url in archives:
                ...
                for game_data in month_games:
                    ...
                    metadata = self._extract_game_metadata_from_pgn(pgn_text, "chess.com")
                    if metadata:
                        games.append(metadata)
                        self.known_game_ids.add(metadata.game_id)
                
        except Exception as e:
            logger.warning(f"⚠️ Error processing Chess.com games for {username}: {e}")
    ...
    def _fetch_games_for_user(self, user_profile: UserProfile, max_games_per_user: int, 
        ...
        try:
            ...
            logger.warning(f"⚠️ Error fetching games for {user_profile.username}: {e}")
            return []
    ...
    def fetch_random_games(self, platform: Platform, skill_level: SkillLevel, 
            ...
            try:
                ...
                for user_data in users_data.get('users', []):
                    profile = UserProfile(
                        ...
                        discovery_date=datetime.fromisoformat(user_data['discovery_date']),
        ...
        logger.info(f"🎯 Targeting {max_games_per_user} games per user from {len(user_profiles)} users")
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(future_to_user):
                ...
                try:
                    ...
                    if user_games:
                        logger.info(f"✅ {user.username} ({user.platform}): {len(user_games)} games")
                    
                    # Stop if we have enough games
                    if len(all_games) >= max_games:
                        logger.info(f"🎯 Target reached! Collected {len(all_games)} games")
                        break
                        
                except Exception as e:
                    logger.warning(f"⚠️ Failed to fetch games for {user.username}: {e}")
    ...
    def save_games(self, games: List[GameMetadata], output_file: str, include_metadata: bool = True):
        """Save games to PGN file with optional metadata."""
...
Examples:
  ...
  python smart_random_games_fetcher.py --users-file discovered_users.json --max-games 300
    ...
    parser.add_argument('--since', help='Fetch games since date (YYYY-MM-DD format)')
    parser.add_argument('--users-file', help='JSON file with pre-discovered users (optional)')
    ...
    if not args.output:
        ...
        args.output = f"{PGN_PATH}/random/smart_random_{platform_str}_{args.skill_level}_{timestamp}.pgn"
    ...
    try:
        ...
        if games:
            ...
            print(f"   - Platforms: {', '.join(set(g.platform for g in games))}")
            print(f"   - Game types: {', '.join(set(g.game_type for g in games))}")
            print(f"   - Rating range: {min(min(g.white_rating, g.black_rating) for g in games if g.white_rating > 0)} - {max(max(g.white_rating, g.black_rating) for g in games if g.white_rating > 0)}")
            print(f"   - Output file: {output_path}")
            
            # Show sample games
            print(f"\n🎮 Sample fetched games:")
            for i, game in enumerate(games[:5]):
                avg_rating = (game.white_rating + game.black_rating) // 2 if game.white_rating > 0 and game.black_rating > 0 else 0
                print(f"   {i+1}. {game.white_player} vs {game.black_player} ({game.platform}) - {avg_rating} avg rating - {game.moves_count} moves")
